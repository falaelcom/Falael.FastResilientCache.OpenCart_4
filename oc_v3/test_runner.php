<?php
/**
 * OpenCart Cache Concurrency Orchestrator (Adapted to OC 3.x, PHP 7.*)
...
function log_msg($id, $msg) {
    // 1. Get current time with microseconds
    $t = microtime(true);
    $micro = sprintf("%06d", ($t - floor($t)) * 1000000);
    
    $milliseconds = substr($micro, 0, 3);
    $timestamp = date("H:i:s", $t) . '.' . $milliseconds;
    ...
    for ($i = 0; $i < 3; $i++) {
        ...
        usleep(rand(1000, 5000)); // Random backoff to resolve lock contention
    }
}
...
function run_worker_ping($id) {
    try {
        ...
        log_msg($id, "Alive.");
        log_msg($id, "DONE.");
    } catch (\Exception $e) { log_msg($id, "ERR: ".$e->getMessage()); }
}
...
function run_worker_laggy_writer($id) {
    try {
        ...
        $cache = new \Cache\File(3600, 'lag_set_init');
        $key = 'race_key';
...
function run_worker_gc_trigger($id) {
    try {
        ...
        $cache = new \Cache\File(3600, 'force_gc');
...
function run_worker_resurrector($id) {
    try {
        ...
        while(time() - $start < 8) {
            ...
            $cache_trigger = new \Cache\File(3600, 'force_gc');
            unset($cache_trigger); 
...
function run_worker_renderer($id) {
    ...
    } catch (\Exception $e) { log_msg($id, "FATAL: ".$e->getMessage()); }
}
